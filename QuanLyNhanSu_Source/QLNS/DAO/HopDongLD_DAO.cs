...
namespace DAO
{
    public class HopDongLD_DAO
    {
        ...
        public static bool Sua(HopDongLD_DTO hd)
        {
            ...
            {
                con = DataProvider.KetNoi();
                string sTruyVan = string.Format("Update tblChucVu set TenHD = '{0}',LoaiHD = '{1}' where MaHD = '{2}'", hd.TenHD,hd.LoaiHD,hd.MaHD);
        ...
        public static bool Them(HopDongLD_DTO hd)
        {
            ...
            {
                con = DataProvider.KetNoi();
                string sTruyVan = string.Format("Insert into tblHopDongLD values (MaHD, TenHD, LoaiHD) values ('{0}','{1}','{2}')", hd.MaHD,hd.TenHD,hd.LoaiHD);
                DataProvider.ThucThiTruyVan(sTruyVan, con);
                DataProvider.Close_KetNoi(con);
                return true;
            }
